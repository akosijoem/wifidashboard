...
<div id="reportbox" name="year" style="background-color:#FFF;padding:15px;">
	
<h2 class="sub-header" id="reportheader" name="reportyear_{{$year}}">
	<em>{{$location}}</em>: {{$year}}</h2>
<div class="row">
		...
		<tbody>
			...
			<tr>
				<td class="hourly text-center bg-info" name="{{($x<=9) ? '0'.$x : $x}}">
					{{date("F Y", strtotime($data['rows'][$x]['date']."-1")) }}</td>
				...
				<td class="text-center">
					@if($data['rows'][$x]['sms_request'] > 0)
						<button type="button" class="btn btn-link smsList" name="REQUEST_{{$year}}-{{($x<=9) ? '0'.$x : $x}}_{{date('F Y', strtotime($year.'-'.$x.'-1'))}}">{{ $data['rows'][$x]['sms_request'] }}</button>
					@else
				...
				<td class="text-center">
					@if($data['rows'][$x]['sms_replied'] > 0)
						<button type="button" class="btn btn-link smsList" name="REPLIED_{{$year}}-{{($x<=9) ? '0'.$x : $x}}_{{date('F Y', strtotime($year.'-'.$x.'-1'))}}">{{ $data['rows'][$x]['sms_replied'] }}</button>
					@else
				...
				<td class="text-center">
					@if($data['rows'][$x]['sms_success'] > 0)
						<button type="button" class="btn btn-link smsList" name="SUCCESSFUL_{{$year}}-{{($x<=9) ? '0'.$x : $x}}_{{date('F Y', strtotime($year.'-'.$x.'-1'))}}">{{ $data['rows'][$x]['sms_success'] }}</button>
					@else
