...
<div id="reportbox" name="month" style="background-color:#FFF;padding:15px;">
	
<h2 class="sub-header" id="reportheader" name="reportmonth_{{date('Ym', strtotime($data['day'].'-1')) }}">
	<em>{{$location}}</em>: {{date("F Y", strtotime($data['day']."-1")) }}</h2>
<div class="row">
	...
	<table class="table table-bordered">
		...
		<tbody>
			...
			<tr>
				<td class="hourly text-center bg-info" name="{{ $data['rows'][$x]['date'] }}">
					{{ $data['rows'][$x]['date'] }}</td>
				...
				<td class="text-center">
					@if($data['rows'][$x]['sms_request'] > 0)
						<button type="button" class="btn btn-link smsList" name="REQUEST_{{$data['day']}}-{{($x<=9) ? '0'.$x : $x}}_{{date('d F Y', strtotime($data['day'].'-'.$x))}}">{{ $data['rows'][$x]['sms_request'] }}</button>
					@else
				...
				<td class="text-center">
					@if($data['rows'][$x]['sms_replied'] > 0)
						<button type="button" class="btn btn-link smsList" name="REPLIED_{{$data['day']}}-{{($x<=9) ? '0'.$x : $x}}_{{date('d F Y', strtotime($data['day'].'-'.$x))}}">{{ $data['rows'][$x]['sms_replied'] }}</button>
					@else
				...
				<td class="text-center">
					@if($data['rows'][$x]['sms_success'] > 0)
						<button type="button" class="btn btn-link smsList" name="SUCCESSFUL_{{$data['day']}}-{{($x<=9) ? '0'.$x : $x}}_{{date('d F Y', strtotime($data['day'].'-'.$x))}}">{{ $data['rows'][$x]['sms_success'] }}</button>
					@else
